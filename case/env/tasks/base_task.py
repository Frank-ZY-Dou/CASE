...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False):
        ...
        # print("check the size of the obs")
        # print(self.num_obs) # ASE 87 motion: 253
        # input("22") # AMP: 224
        ...
        # This is for CGAN
        self.label_buf = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.long)  # this is for the label
        # allocate buffers
        self.obs_buf = torch.zeros(
            (self.num_envs, self.num_obs), device=self.device, dtype=torch.float)
        #224 with condition label (1)
